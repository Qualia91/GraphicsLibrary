...
public class CameraRemoveEvent implements RenderUpdateEvent<Camera> {
	...
	public CameraRemoveEvent(InstanceObject instanceObject, Camera camera, String layerName) {
	...
	public void applyToGraphicsEngine(Window window) {
		...
		window.getRenderGraphs().get(layerName).getCameras().remove(camera);
	}
}
